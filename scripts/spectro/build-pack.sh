...
| Pack chart archive sha256 | \`$(sha256sum "$pack_dir/$chart_archive" | cut -d' ' -f1)\` (deterministic) |
...
- Repackaged \`$chart_archive\` reports chart version \`$version\`, so the archive name and its contents agree, and \`helm show chart\` can read it.
- The pack archive is repacked deterministically, so rebuilding the same version reproduces the same bytes.
